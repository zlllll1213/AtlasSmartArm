...
- 不把后端 mock/辅助库存接口描述为当前真机自动库存闭环。
- 不实现真实权限系统。
- 不上传 SSH 密码、模型权重或拍照样本到仓库。
- 不上传本地 Agent/Claude 配置，例如 `.claude/`、`CLAUDE.md`、`Agent.md`。
...
| `GET` | `/api/v1/camera/captures/{capture_id}/image` | 读取拍照图片 |
| `WS` | `/ws/v1/events` | 事件流 |

### 6.2 辅助和保留接口

以下 HTTP 接口在后端实现并纳入契约测试或后续扩展，但不代表当前真机 demo 已实现自动库存闭环或独立硬件急停：

| 方法 | 路径 | 当前定位 |
| --- | --- | --- |
| `POST` | `/api/v1/vision/detect` | Mock/后续模型接入检测 |
| `GET` | `/api/v1/calibration/status` | 标定状态 |
| `PUT` | `/api/v1/calibration/offset` | 抓取偏移配置 |
| `POST` | `/api/v1/arm/kinematics/fk` | FK 校验 |
| `POST` | `/api/v1/arm/kinematics/ik` | IK 校验 |
| `POST` | `/api/v1/arm/move` | 运动请求校验或 mock 执行 |
| `POST` | `/api/v1/arm/emergency-stop` | 软件态停止标记，不等同于硬件急停 |
| `GET` | `/api/v1/inventory/items` | 内存态库存列表 |
| `POST` | `/api/v1/inventory/items` | 内存态库存创建 |
| `GET` | `/api/v1/inventory/items/{item_id}` | 内存态库存查询 |
| `PUT` | `/api/v1/inventory/items/{item_id}` | 内存态库存更新 |
| `POST` | `/api/v1/inventory/inbound` | 手动入库调整 |
| `POST` | `/api/v1/inventory/outbound` | 手动出库调整 |
| `POST` | `/api/v1/inventory/audit` | 手动盘点调整 |
