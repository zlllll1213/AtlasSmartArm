...
本项目没有从零编写自定义训练脚本，而是调用 YOLOv5 官方仓库自带的 Python 脚本 `train.py` 做迁移训练。训练前需要准备好图片、YOLO 标注文件、数据集 YAML 和 `yolov5s.pt` 预训练权重；`train.py` 内部负责读取数据、加载预训练模型、将检测头从 80 类改成 16 类、执行数据增强、计算 loss、反向传播、验证并保存 `best.pt`。
...
也就是说，训练入口是官方 `train.py`，本项目的工作重点是数据整理、类别顺序配置、训练参数选择、训练结果导出和开发板部署。
